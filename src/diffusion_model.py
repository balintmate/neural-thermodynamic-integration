...
class diffusion_model:
    ...
    @partial(jax.jit, static_argnames=["self", "num_samples", "n"])
    def sample_traj(self, params, key, num_samples, n):
        x = jax.random.uniform(key, (num_samples, n, self.num_features))
        x_traj = jnp.tile(jnp.expand_dims(x, 0), (self.num_integration_steps, 1, 1, 1))
        dlogZ_traj = jnp.zeros((self.num_integration_steps, num_samples))
        n = jnp.array([n])
        dt = 1 / self.num_integration_steps
        t = jnp.array([1.0])
        init_value = (x_traj, dlogZ_traj, t, key)

        def body_fun(i, carry):
            x, dlogZ, t, key = carry
            x_curr = x[i]
            z = jax.random.normal(key, x_curr.shape)
            ...
            score, dlogZ_one = jax.vmap(force_dlogZ, in_axes=(0, None))(x_curr, t)

            drift = self.beta(t[0]) * score
            x_curr += drift * dt + z * (self.beta(t[0]) * dt) ** 0.5
            x_curr = x_curr % 1
            x = x.at[i + 1].set(x_curr)
            dlogZ = dlogZ.at[i].set(dlogZ_one.reshape(-1))
            return x, dlogZ, t - dt, jax.random.split(key)[0]

        x, dlogZ_traj, t, key = jax.lax.fori_loop(
            0, self.num_integration_steps, body_fun, init_value
        )
